...
class FGM_YOLO(nn.Module):
    """适配 YOLO11 的频域引导模块 (Frequency Guided Module)"""
...
class OmniKernelBlock(nn.Module):
    ...
    结合了超大核深度可分离卷积与频域注意力
    """
    def __init__(self, c1, c2, ker=31): 
        super().__init__()
        dim = c2
        pad = ker // 2
        
        # 【新增】：通道对齐层。如果 c1 != c2，用 1x1 卷积降维/升维；如果相等，则什么都不做 (Identity)
        self.shortcut = nn.Conv2d(c1, dim, kernel_size=1, stride=1, bias=False) if c1 != dim else nn.Identity()

        # 统一通道数
        self.in_conv = nn.Sequential(
            nn.Conv2d(c1, dim, kernel_size=1, padding=0, stride=1),
            nn.GELU()
        )
        ...
        self.fgm = FGM_YOLO(dim)

    def forward(self, x):
        # 【修改】：首先计算捷径分支，确保用于残差相加的 base 张量通道数为 dim
        ...
        # 【修改】：用 base 替换原来的 x，确保所有的特征通道数均为 128 (dim)
        out = base + self.dw_13(out) + self.dw_31(out) + self.dw_33(out) + self.dw_11(out) + x_sca
        out = self.act(out)
