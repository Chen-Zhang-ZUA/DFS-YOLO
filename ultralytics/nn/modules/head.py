...
class PPH_Detect(nn.Module):
    """
    高保真轻量化解耦检测头 (Precision-Preserving Head)
    使用深度可分离架构 (DW+PW) 替代标准密集卷积，旨在不损失 mAP 的前提下极限压缩参数与 GFLOPs。
    """
    dynamic = False  
    export = False  
    shape = None
    ...
    def __init__(self, nc=80, ch=()):  
        ...
        # 保持原生 YOLO 的通道维度计算逻辑，保证特征表达能力不缩水！
        ...
        for x in ch:
            ...
            self.cv2.append(nn.Sequential(
                # Block 1: 3x3 Depthwise (空间) + 1x1 Pointwise (通道)
                Conv(x, x, 3, g=x),  
                Conv(x, c2, 1),      
                # Block 2: 3x3 Depthwise (空间) + 1x1 Pointwise (通道)
                Conv(c2, c2, 3, g=c2),
                Conv(c2, c2, 1),
                # 输出层
