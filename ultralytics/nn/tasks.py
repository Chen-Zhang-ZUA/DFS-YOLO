...
from ultralytics.nn.block.sca_fusion import SCA_Fusion
from ultralytics.nn.modules.block import OmniKernelBlock
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is C3k2 and scale in "mlx":  # for M/L/X sizes
                    args[3] = True
            if m is A2C2F_CLASS and scale in "lx": # for L/X sizes
                args.extend((True, 1.2))
            if m is DMB:
                # Signature: c1, c2, wm, wm_name, k, s, p, g, d, act
                args.insert(2, warehouse_manager) # index 2 -> wm
                args.insert(3, f'layer{i}')       # index 3 -> wm_name
            elif m is C3k2_DMB:
                # Signature: c1, c2, n, wm, wm_name, c3k, e, g, shortcut
                # Note: 'n' was already inserted at index 2 by the repeat_modules block
                args.insert(3, warehouse_manager) # index 3 -> wm
                args.insert(4, f'layer{i}')       # index 4 -> wm_name
        elif m is AIFI:
            args = [ch[f], *args]
            c2 = args[0]
        ...
        elif m in {SpatialAttention, CBAM, CSPOKnet}:
            c2 = ch[f]
            args = [c2, *args]
            # print(args)
        
        elif m is SCA_Fusion:
            c2 = ch[f[1]]
            args = [c2, *args]
       
        elif m in {OmniKernelBlock}:
            c1, c2 = ch[f], args[0]
